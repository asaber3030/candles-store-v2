        ...
        <div className='grid md:grid-cols-2 gap-12 items-center'>
          ...
          <div className='flex flex-col justify-center space-y-8'>
            <div className='space-y-4'>
              <h1 className='text-5xl sm:text-6xl lg:text-7xl font-light text-foreground text-balance'>{t("hero.title")}</h1>
              <p className='text-lg sm:text-xl text-muted-foreground text-balance leading-relaxed'>{t("hero.subtitle")}</p>
            </div>

            <div className='flex flex-col sm:flex-row gap-4'>
              <Link href={userRoutes.shop} className='px-8 py-3 bg-primary text-primary-foreground rounded-lg font-medium hover:bg-primary/90 transition-colors text-center'>
                {t("hero.cta")}
              </Link>
              <Link href={userRoutes.about} className='px-8 py-3 border border-primary text-primary rounded-lg font-medium hover:bg-primary/10 transition-colors text-center'>
                {t("Learn More")}
              </Link>
            </div>
          </div>
          ...
          <div className='relative h-96 md:h-[500px] from-primary/20 to-accent/20 rounded-2xl overflow-hidden flex items-center justify-center'>
            <div className='relative w-64 h-64 md:w-80 md:h-80'>
              <img src='/images/candles/05.jpg' alt='Featured candle' className='w-full h-full object-cover rounded-xl' />
            </div>
          </div>
        </div>
      </DefaultContainer>
