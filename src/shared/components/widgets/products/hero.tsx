...
export const ProductsHero = ({ section }: Props) => {
  ...
  return (
    <section className="relative overflow-hidden border-b bg-gradient-to-br from-amber-100 via-rose-100 to-purple-100 py-32">
      ...
      <div className="absolute inset-0 opacity-30">
        ...
        <div
          className="absolute bottom-20 right-20 w-[500px] h-[500px] bg-gradient-to-br from-rose-400 to-pink-400 rounded-full blur-3xl animate-pulse"
          style={{ animationDelay: "1s" }}
        ></div>
        <div
          className="absolute top-1/2 left-1/2 w-72 h-72 bg-gradient-to-br from-purple-400 to-indigo-400 rounded-full blur-3xl animate-pulse"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center">
          ...
          <div className="inline-flex items-center gap-2 bg-white/90 backdrop-blur-sm px-6 py-3 rounded-full mb-8 shadow-lg animate-bounce-slow">
            ...
            <span className="text-sm font-bold text-transparent bg-clip-text bg-gradient-to-r from-amber-600 to-rose-600">
              {t("productsPage.hero.badge")}
            </span>
          </div>

          {/* Animated Title */}
          <h1 className="text-6xl md:text-8xl font-bold text-stone-900 tracking-tight animate-fade-in">
            <span className="inline-block animate-slide-up" style={{ animationDelay: "0.1s" }}>
              {usedTranslation.title ? usedTranslation.title : t("productsPage.hero.title").split(" ")[0]}
            </span>{" "}
            <span
              className="inline-block text-transparent bg-clip-text bg-gradient-to-r from-amber-600 to-rose-600 animate-slide-up"
              style={{ animationDelay: "0.4s" }}
            >
              {usedTranslation.title ? usedTranslation.title.split(" ")[3] : t("productsPage.hero.title").split(" ")[3]}
            </span>
          </h1>

          <p className="text-2xl text-stone-700 max-w-3xl mx-auto mb-12 animate-fade-in" style={{ animationDelay: "0.5s" }}>
            {usedTranslation.content ? usedTranslation.content : t("productsPage.hero.subtitle")}
          </p>
