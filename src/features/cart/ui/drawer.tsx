  ...
  return (
    <Sheet>
      <SheetTrigger asChild>
        <Button className="relative size-10 rounded-full" icon={ShoppingCart} size="icon">
          {totalQuantity > 0 && (
      ...
      <SheetContent side="right" className="w-[400px] p-4">
        <SheetHeader>
          ...
          {items.length === 0 ? (
            <SheetDescription>{t("Your cart is empty")}</SheetDescription>
          ) : (
            <SheetDescription>{t("Review your selected products")}</SheetDescription>
          )}
        </SheetHeader>
