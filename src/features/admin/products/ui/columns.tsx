...
import { AddToFeaturedProductsModal } from "./is-featured-modal";
...
export const ProductsColumns: ColumnDef<ProductWithCategory>[] = [
  ...
  },
  {
    ...
    header: "Featured",
    cell: ({ row }) => <Badge variant={row.getValue("isFeatured") ? "success" : "default"}>{row.getValue("isFeatured") ? "Yes" : "No"}</Badge>,
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex gap-2">
          ...
          <AddToFeaturedProductsModal productId={product.id} isFeatured={product.isFeatured} />
        </div>
      );
    },
  },
