...
export function DeliveryOrderTracker({ currentStatus = OrderStatusEnum.JustOrdered }: { currentStatus: OrderStatusEnum }) {
    ...
    <div className='w-full max-w-3xl'>
      ...
      <ol className='flex items-center w-full'>
        {OrderStatusList.map((status, index) => {
          ...
          return (
            <li key={status} className={`flex items-center ${index !== OrderStatusList.length - 1 ? "w-full" : ""}`}>
